...
class ListTitlesAdapter(
...
) : ListAdapter<String, ListTitlesAdapter.Vh>(DiffCallback()) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Vh {
        return Vh(
            parent, onItemClick = mItemCLickListener,
    ...
    class Vh(
        parent: ViewGroup, onItemClick: (String) -> Unit,
        onEditClick: (String) -> Unit,
        onDeleteClick: (String) -> Unit
    ) : RecyclerView.ViewHolder(
        LayoutInflater.from(parent.context).inflate(R.layout.item_title_llist_rv, parent, false)
    ) {
